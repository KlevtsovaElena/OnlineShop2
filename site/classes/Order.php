...
class Order extends \AbstractClasses\Unit
{
    ...
    //функция добавления нового заказа в таблицу order
    ...
    //функция добавления данных корзины этого заказа в таблицу order_items
    public static function createOrderItems() : mixed
    {
         
        $pdo = \Connection::getConnection();

        //определяем id нашего заказа по id user и дате оформления заказа, результат запишем в $idOrder
        ...
        //теперь запишем через цикл полученные данные корзины пользователя в таблицу  Cart и отредактируем поле resrve в таблице goods
